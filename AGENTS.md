...
- `"einleitung"`: injected instruction for concrete Einstieg → Forschungsfrage → Aufbau structure. The chapter count is dynamically computed (`topLevelChapterCount = abschnitte.filter(s => !s.nummer.includes(".")).length`) in `generate-content/route.ts` and threaded through `writeSection` → `buildSectionPrompt` → `buildSectionTypeInstruction` so the Einleitung always states the correct number of chapters (e.g. "vier Kapitel", never a hardcoded "fünf"). The Aufbau instruction requires **exact chapter titles from the Gliederung** — not paraphrased, not summarized — to prevent the Einleitung from misdescribing chapters (e.g. writing "betriebliche Relevanz" when the actual chapter is "Kritische Diskussion"). A `HAUPTKAPITEL ZUR REFERENZ` block is injected with a literal list of `Kapitel N: Titel` entries from `topLevelSections` (filtered from `abschnitte` in both generate-content and review-content routes).
- `"fazit"`: injected instruction for Kernbefunde → Limitationen → Ausblick + explicit `WORTLIMIT: maximal N Wörtern`. Two additional guards: **Zahlenkonsistenz** (all statistics in the Fazit must exactly match the values written in the Hauptteil — no rounding or paraphrasing; when a study has multiple values for different conditions, ALL must appear in the Fazit); **Studiendesign-Begriffe** (methodological terms like "Querschnittsdesign" are forbidden unless those exact terms were used to describe the studies in the Hauptteil — prevents the model from applying generic limitation templates to studies actually described as field studies or RCTs; does NOT apply to future-research recommendations in the Ausblick).
...
**Pass 3 — Kapitelkopf detection**: among sections typed `"hauptteil"`, any section where `ebene === 1 && nextSection.nummer.startsWith(s.nummer + ".")` is reclassified as `"kapitelkopf"` and its `geschaetzteWorte` is forced to 40. These are chapter-heading sections whose real content lives entirely in their subsections. The word-count gate (`<= 60`) was intentionally removed: the outline agent assigns large word targets to parent chapters (e.g. 523w for "Theoretische Grundlagen"), causing them to miss detection and write 500w intros that duplicate their subsections — triggering 10+ review rewrites per run. Forcing `geschaetzteWorte: 40` lets Pass 4 redistribute those words to the real content sections. With detection, they get a tight token budget and a prompt to write 1–2 sentences only.
